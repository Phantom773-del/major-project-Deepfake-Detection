...
def test_default_registry_has_only_implemented_stages() -> None:
    registry = build_default_registry()
    assert registry.names() == ["validate", "fingerprint"]
    assert registry.get("metadata") is None
    assert "metadata" not in registry
...
async def test_worker_processes_queued_scan_to_completed(
    ...
    for future in ("metadata", "detect", "forensics", "xai", "evidence",
...
async def test_api_scan_reaches_completed_via_worker(
    ...
    assert stages["metadata"]["status"] == "SKIPPED"
